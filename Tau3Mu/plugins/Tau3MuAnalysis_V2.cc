...
private:
  ...
  virtual Vertex findPV(const edm::Event&, TransientVertex&, TLorentzVector&, bool&);
  virtual void vtx(const edm::Event&, const edm::EventSetup&, std::vector<TransientTrack>&, TVector3*, TVector3*, bool &, double &, double &, int, double*);
  ...
  virtual bool isTight(const reco::Muon*, Vertex&);
  virtual bool TriggerDecision(const edm::Event&);
  virtual bool getMu(const edm::Event&, Track&, Muon&);
  virtual int countTracksAround(const edm::Event&, const edm::EventSetup&, TLorentzVector*, double&, TransientVertex&, int, double&);
  ...
  virtual bool isBkgMatched(const edm::Event&,TLorentzVector*, double*);
  virtual bool isInPV(Vertex&,double);
  virtual double sumPT(Vertex&);
  virtual bool isInOtherPV(const edm::Event&, Vertex&, double);
  virtual void TrackCompatibility(const edm::EventSetup&,Vertex&, TransientVertex&, vector<TransientTrack>&, int&, TLorentzVector&);
  virtual void ptAround(const edm::Event&, TransientVertex&, Vertex&, vector<TransientTrack>&, TLorentzVector&, double, double&, int&);
  virtual double Compute_CosPointingAngle(Vertex& , TransientVertex& ,TLorentzVector&);
  virtual double Compute_CosPointingAngle3D(Vertex& , TransientVertex& ,TLorentzVector&);
  ...
  virtual void setMuIdToTrack(const edm::Event&, const Track*,bool&,bool&,bool&,bool&,bool&,bool&,int&,int&,int&,int&,double&,double&, bool&, double&, Vertex&);
  virtual void setMuIdToMu(Muon &,bool&,bool&,bool&,bool&,bool&,bool&,int&,int&,int&,int&,double&,double&,Vertex&);
  virtual void findTriggerObjects(const edm::Event&, const edm::EventSetup&, std::vector<TLorentzVector>&, int);
  virtual void vertexingTracks(const edm::Event&, const edm::EventSetup&, TransientVertex&, int&, double*);
  virtual void vertexingTracksAdaptive(const edm::Event&, const edm::EventSetup&, TransientVertex&, int&, TVector3*, double*);
  virtual bool highestWeight(const edm::Event&, const edm::EventSetup&, TransientVertex&, double);
  virtual int  sharedSegmentsTkMu( const reco::Muon&, const reco::Muon&);
  virtual void DumpMother( const Candidate*, TVector3*);
  ...
  TVector3 *_SVTAR;
  double _w1R,_w2R,_w3R;

  TVector3 *_SV_Mu1TR, *_SVe_Mu1TR,*_SV_Mu2TR,*_SVe_Mu2TR;

  TVector3 *_SVA_Mu1TR_Redone,*_SVA_Mu2TR_Redone,*_SVTAR_Redone,*_SVAR_Redone;

  bool _svValidR,_svtValidR, _isPVLeadingR;

  bool _sv1tValidR, _sv2tValidR;

  bool _inPVR1,_inPVR2,_inPVR3;
  bool _inOPVR1,_inOPVR2,_inOPVR3;
  bool _tkHighestWR;
  ...
  double _cosp23DR,_cosp33DR;

  int _Ntracks_Mu1_R,_Ntracks_Mu2_R,_Ntracks_Mu3_R;
  double _SumPtMu1R,_SumPtMu2R,_SumPtMu3R;

  int _NtkPVR;
  double _SumPtPVR;

  int _Ntracks_SVT_R, _Ntracks_SV1_R, _Ntracks_SV2_R, _Ntracks_SV_R;
  int _Ntracks_SVT_RA, _Ntracks_SV1_RA, _Ntracks_SV2_RA,_Ntracks_SV_RA;
  int _NtracksPV_SVT_R;

  double _Sum07R,_Sum03R,_Sum05R;
  int _Num07R,_Num03R,_Num05R;
  ...
  double _3D_IP1_SVTR,_3D_IP1Sig_SVTR ;
  double _3D_IP2_SVTR,_3D_IP2Sig_SVTR ;
  
  double _3D_IP_SVTR,_3D_IPSig_SVTR ;
  double _3D_IP_SVR,_3D_IPSig_SVR ;
  double _3D_IP_PVR,_3D_IPSig_PVR ;

  double _3D_IP1_PVR,_3D_IP1Sig_PVR ;
  double _3D_IP2_PVR,_3D_IP2Sig_PVR ;
  ...
  TVector3 *_SVA_Mu1T_Redone,*_SVA_Mu2T_Redone,*_SVTA_Redone,*_SVA_Redone;
  ...
  bool _inPV1,_inPV2,_inPV3;
  bool _inOPV1,_inOPV2,_inOPV3;
  bool _tkHighestW;

  double _cosp2,_cosp3;
  double _cosp23D,_cosp33D;

  int _NtkPV;
  double _SumPtPV;

  int _Ntracks_Mu1,_Ntracks_Mu2,_Ntracks_Mu3;
  double _SumPtMu1,_SumPtMu2,_SumPtMu3;
  int _Ntracks_SVTA, _Ntracks_SV1A, _Ntracks_SV2A,_Ntracks_SVA;
  int _Ntracks_SVT, _Ntracks_SV1, _Ntracks_SV2,_Ntracks_SV;
  int _NtracksPV_SVT;

  double _Sum07,_Sum03,_Sum05;
  int _Num07,_Num03,_Num05;
  ...
  double _3D_IP1_SVT,_3D_IP1Sig_SVT ;
  double _3D_IP2_SVT,_3D_IP2Sig_SVT ;

  double _3D_IP_SVT,_3D_IPSig_SVT ;
  double _3D_IP_SV,_3D_IPSig_SV ;
  double _3D_IP_PV,_3D_IPSig_PV ;

  double _3D_IP1_PV,_3D_IP1Sig_PV ;
  double _3D_IP2_PV,_3D_IP2Sig_PV ;
...
bool Tau3MuAnalysis_V2::isInOtherPV(const edm::Event& ev, Vertex& pv, double pt){
  ...
  for (VertexCollection::const_iterator pvtmp=pvHandle->begin(); pvtmp!=pvHandle->end(); pvtmp++){
    if (pvtmp->z()==pv.z()) continue;

    for(std::vector<reco::TrackBaseRef>::const_iterator it = pvtmp->tracks_begin() ; it != pvtmp->tracks_end(); ++it ){
      if (!(it->isNonnull() && it->isAvailable())) continue;
      Track tr=*(it->get());
      if (tr.pt()==pt) itIs=true;
    }
  }
...
void Tau3MuAnalysis_V2::DumpMother( const Candidate * cand, TVector3* vec){
 
  const Candidate* tmp=cand;
  bool status=true;
  double x,y,z;

  while (status){  
    tmp=tmp->mother();
    //cout << "id " << tmp->pdgId() << " st " << tmp->status() << " vz " << tmp->vz()<<endl;
    x=tmp->vx();
    y=tmp->vy();
    z=tmp->vz();
    
    if (abs(tmp->pdgId())<6 || tmp->pdgId()==21 || tmp->mother()->status()==3) status=false;
  }
  vec->SetXYZ(x,y,z);
}

Vertex Tau3MuAnalysis_V2::findPV(const edm::Event& ev,TransientVertex& sv, TLorentzVector& mom, bool& leading){
  ...
  for (VertexCollection::const_iterator pvtmp=pvHandle->begin(); pvtmp!=pvHandle->end(); pvtmp++){
    countvtx++;
    Vertex vtmp=Vertex(*pvtmp);
    if(Compute_CosPointingAngle3D(vtmp,sv,mom) > cos3Max){
      cos3Max=Compute_CosPointingAngle3D(vtmp,sv,mom) ;
      pVertex=vtmp;
    }
  }
  if (countvtx==1) leading=true;

  if (pVertex.x()==0 && pVertex.y()==0 && pVertex.z()==0){
    if (debug || 1) cout << "No PV found with pointing, adding it with dZ" << endl;
    pVertex=findClosestPV(ev,sv,leading);
  }
...
bool Tau3MuAnalysis_V2::isTight(const Muon* recoMu, Vertex& vtx){
  ...
  bool ip = fabs(recoMu->muonBestTrack()->dxy(vtx.position())) < 0.2 && fabs(recoMu->muonBestTrack()->dz(vtx.position())) < 0.5;
  if (!ip) isTight=false;
...
void Tau3MuAnalysis_V2::findBestPiCand(const edm::Event& ev, const edm::EventSetup& iSetup, MuonCollection& dimu ,TransientVertex& tv, Vertex& primaryVertex,TransientVertex& dimuvtx , TLorentzVector& pi, Track& track){
  ...
  if (trackFound) {
    setMuIdToTrack(ev, &track, _isTk3,_isSA3,_isGlb3 ,_isTMO3, _isPF3, _isTight3, _npix3,_nTk3,_nMu3,_q3, _chiTk3,_chiMu3, shares, _kink3,primaryVertex);

   _inPV3=isInPV(primaryVertex,track.pt());
   _inOPV3=isInOtherPV(ev,primaryVertex,track.pt());

  }
  if (_kink3==999) _kink3=-1;

  _NtkPV=primaryVertex.nTracks();
  _SumPtPV=sumPT(primaryVertex);
}
...
void Tau3MuAnalysis_V2::ptAround(const edm::Event& ev,TransientVertex& sv,Vertex& pv,vector<TransientTrack>& vtt, TLorentzVector& trimu, double dR, double& sumPt, int& Ntk){

  //count n tracks and sumPt around the tau candidate
  ...
  for(std::vector<reco::TrackBaseRef>::const_iterator it = pv.tracks_begin() ; it != pv.tracks_end(); ++it ){
    ...
    if (trv.DeltaR(trimu) > dR) continue;
      sumPt += tr.pt();
      Ntk++;
  ...
  for(TrackCollection::const_iterator it = tracks->begin();it != tracks->end(); ++it){
    ...
    if (!(it->quality(TrackBase::highPurity))) continue;
    if (it->pt() < MinTrackPt) continue;
    if (it->dz(Vertex(sv).position())>0.05) continue;
    bool isInPv=false;
    for(std::vector<reco::TrackBaseRef>::const_iterator pvit = pv.tracks_begin() ; pvit != pv.tracks_end(); ++pvit ){
      Track tr=*(pvit->get());
      if (it->pt()==tr.pt()) isInPv=true;
    }

    TLorentzVector trv=TLorentzVector(it->px(),it->py(),it->pz(),sqrt(TrackMass*TrackMass+it->p()*it->p()));
...
void Tau3MuAnalysis_V2::TrackCompatibility(const edm::EventSetup& iSetup, Vertex & pv, TransientVertex& sv, vector<TransientTrack>& vtt, int& ncomp, TLorentzVector& trimu){
  ...
  for(std::vector<reco::TrackBaseRef>::const_iterator it = pv.tracks_begin() ; it != pv.tracks_end(); ++it ){
    ...
    if (tr.pt() < 1.0) continue;
    if (tr.hitPattern().numberOfValidPixelHits()<2) continue;
    if (tr.pt()==vtt[0].track().pt() || tr.pt()==vtt[1].track().pt() || tr.pt()==vtt[2].track().pt()) continue;
   
    TLorentzVector vtr=TLorentzVector(tr.px(),tr.py(),tr.pz(),1);
    TLorentzVector tot=trimu+vtr;
    double cosPoint=Compute_CosPointingAngle3D(pv,sv,tot);

    if (cosPoint < 0.8) continue;
...
double Tau3MuAnalysis_V2::sumPT(Vertex& pv){
...
bool Tau3MuAnalysis_V2::isInPV(Vertex& pv,double pt){
...
void Tau3MuAnalysis_V2::findGenMoms(const edm::Event& ev, std::vector<TLorentzVector>& TheGenMus){
 ...
 //std::vector<TLorentzVector> TheGenMus;
  ...
  if (!isSignal){
	...
	if (dauId==333){
	  ...
	  for(int k1 = 0; k1 < ndauphi; ++ k1) {
		...
		DumpMother(&cand,_GenPV);
		/*cout << "Dsvtx x " << Dsvtx.X() << " y " << Dsvtx.Y() << " z " << Dsvtx.Z() << endl;
		cout << "Mother Ds x " << cand.mother()->vx() << " y " << cand.mother()->vy() << " z " << cand.mother()->vz() << " id " <<cand.mother()->pdgId() <<endl;
		cout << "Mother Mother Ds x " << cand.mother()->mother()->vx() << " y " << cand.mother()->mother()->vy() << " z " << cand.mother()->mother()->vz() << " id " <<cand.mother()->mother()->pdgId() <<endl;
		cout << "Mother Mother Mother Ds x " << cand.mother()->mother()->mother()->vx() << " y " << cand.mother()->mother()->mother()->vy() << " z " << cand.mother()->mother()->mother()->vz() << " id " <<cand.mother()->mother()->mother()->pdgId() <<endl;*/
  ...
  TVector3 vdimu=TVector3(dimu.Px(),dimu.Py(),0.);
  TVector3 vtrimu=TVector3(trimu.Px(),trimu.Py(),0.);


  TVector3 Displacement=TVector3(_GenSV->X()-_GenPV->X(),_GenSV->Y()-_GenPV->Y(),0.);

  _lxyG=((Displacement.X()*trimu.Px())+(Displacement.Y()*trimu.Py()))*trimu.M()/pow(trimu.Pt(),2);

  _cosp2G=(Displacement.X()*vdimu.Px()+Displacement.Y()*vdimu.Py())/(vdimu.Mag()*Displacement.Mag());
  _cosp3G=(Displacement.X()*vtrimu.Px()+Displacement.Y()*vtrimu.Py())/(vtrimu.Mag()*Displacement.Mag());

  Displacement.SetZ(_GenSV->Z()-_GenPV->Z());

  _cosp23DG=(Displacement.X()*dimu.Px()+Displacement.Y()*dimu.Py()+Displacement.Z()*dimu.Pz())/(dimu.Vect().Mag()*Displacement.Mag());
  _cosp33DG=(Displacement.X()*trimu.Px()+Displacement.Y()*trimu.Py()+Displacement.Z()*trimu.Pz())/(trimu.Vect().Mag()*Displacement.Mag());
...
bool Tau3MuAnalysis_V2::isMcMatched(const edm::Event& ev,TLorentzVector* recov,std::vector<TLorentzVector>& TheGenMus){
  ...
  if (debug) cout << "Matching to offline objects" << endl;

  const double dRIni=0.01;
  ...
  while (RunMatch){
	...
	if (dR < dRtmp){
	  if (debug) cout << "Mc Offline match found dR " << dR << endl;
	  dRtmp=dR;
	  indR=r;
	  indG=g;
	}
  ...
  if (debug){
    for (uint i=0; i<genIndexes.size(); i++){
      ...
      cout << "---- OfflineMom: pT= " << recov[recoIndexes[i]].Pt() << " eta= " << recov[recoIndexes[i]].Eta() << " phi= " << recov[recoIndexes[i]].Phi() << endl;
    }
  }

  if (genIndexes.size()==3)  {ThreeMatches=true;if (debug) cout << "Offline-Mc Matched!" << endl;}
...
void Tau3MuAnalysis_V2::vertexingTracks(const edm::Event& ev, const edm::EventSetup& iSetup, TransientVertex& vtx, int& ntracks, double* pts){
 
  std::vector<TransientTrack> vtt=vtx.originalTracks();
  int originalSize= vtt.size();

  int ptSize=sizeof(pts)/sizeof(pts[0]);
  ...
  for(TrackCollection::const_iterator it = tracks->begin();it != tracks->end(); ++it){
    ...
    if (!(it->quality(TrackBase::highPurity))) continue;
    if (it->pt() < MinTrackPt) continue;
    if (it->dz(Vertex(vtx).position())>0.2) continue;

    bool isIn=false;

    for (uint k=0; k< vtt.size(); ++k){
      if (vtt[k].track().pt()==it->pt()) isIn=true;     
    }
    ...
    for (int k=0; k<ptSize; k++){
      if (it->pt()==pts[k]) isSelected=true;
    }
...
void Tau3MuAnalysis_V2::vertexingTracksAdaptive(const edm::Event& ev, const edm::EventSetup& iSetup, TransientVertex& vtx, int& ntracks, TVector3* NewPosition, double* pts){
 
  if (debug) cout << "counting additional tracks with adaptive" << endl;
  ...
  TransientVertex iniVtx=avf.vertex(vtt);
  if (!iniVtx.isValid() || !(iniVtx.hasTrackWeight())) return;

  std::vector<double> iniWeights;
  for (uint k=0; k < vtt.size(); k++){
    iniWeights.push_back(iniVtx.trackWeight(vtt[k]));
  }
  ...
  for(TrackCollection::const_iterator it = tracks->begin();it != tracks->end(); ++it){
    ...
    if (!(it->quality(TrackBase::highPurity))) continue; 
    if (it->pt() < MinTrackPt) continue;
    if (it->dz(Vertex(vtx).position())>0.25) continue;
    ...
    if (!(tmpVtx.hasTrackWeight())) continue;
    double lastW=tmpVtx.trackWeight(t);

    if (lastW < 0.5) continue;

    bool weightsOk=true;

    for (int k=0; k< originalSize; ++k){
      if (tmpVtx.trackWeight(vtmp[k]) < 0.85*iniWeights[k]) weightsOk=false;//control that the original tracks have at least the 85% of the original weight in the new fit
    }
  ...
  int finalSize=vtt.size();
  ntracks=finalSize-originalSize;
  NewPosition->SetXYZ(x,y,z);

}

bool Tau3MuAnalysis_V2::highestWeight(const edm::Event& ev, const edm::EventSetup& iSetup, TransientVertex& vtx, double ptTk){
 
  if (debug) cout << "searching highest weight track" << endl;
  ...
  TransientVertex iniVtx=avf.vertex(vtt);
  if (!iniVtx.isValid() || !(iniVtx.hasTrackWeight())) return false;
...
int Tau3MuAnalysis_V2::countTracksAround(const edm::Event& ev, const edm::EventSetup& iSetup,TLorentzVector* vec, double& dR, TransientVertex& sv, int index, double& ptsum ){
 
  ptsum=0;
  if (debug) cout << "counting tracks around mu" << index << endl;

  int N=0;
  Handle<TrackCollection> tracks;
  ev.getByLabel("generalTracks", tracks);
  TLorentzVector Mom= vec[index];
  ...
  for(TrackCollection::const_iterator it = tracks->begin();it != tracks->end(); ++it){
    ...
    if (dz > 0.05) continue; //The track is not close to SV
...
void Tau3MuAnalysis_V2::findBestDimuon(const edm::Event& event, const edm::EventSetup& iSetup,MuonCollection& muIn, MuonCollection& dimu, TransientVertex& dimuvtx, Vertex& primaryVertex){
  ...
  for(uint i=0; i < (muIn.size()-1); ++i){
    ...
    for (uint j=i+1; j<muIn.size(); ++j){
      ...
      Vertex primaryVertexTMP=findPV(event,tmpvtx,DiMu,isPVLeading);

      double cos2=Compute_CosPointingAngle(primaryVertexTMP,tmpvtx,DiMu);
     
      if( cos2 < diMuCosPointMin) continue;


      if (passcos && triggered) {ndmcos++;passcos=false;}

      pair<double,double> lxytmp=Compute_Lxy_and_Significance(primaryVertexTMP,tmpvtx,DiMu);
      ...
      two=j;
      dimuvtx=tmpvtx;            
      primaryVertex=primaryVertexTMP;
    }
  }
...
double  Tau3MuAnalysis_V2::Compute_CosPointingAngle3D(Vertex& PV, TransientVertex& sv,TLorentzVector& DiMuMom){

  TVector3 pperp;
  pperp.SetXYZ(DiMuMom.Px(),DiMuMom.Py(),DiMuMom.Pz());

  TVector3 Displacement=TVector3(sv.position().x()-PV.x(),sv.position().y()-PV.y(),sv.position().z() - PV.z());

  double cosAlpha=(Displacement.X()*pperp.X()+Displacement.Y()*pperp.Y() + Displacement.Z()*pperp.Z() )/(pperp.Mag()*Displacement.Mag());
  return cosAlpha;
}
...
void Tau3MuAnalysis_V2::vtx(const edm::Event& event, const edm::EventSetup& iSetup, std::vector<TransientTrack>& tt, TVector3* p, TVector3* ep, bool & valid, double & chi2, double & prob, int muNumber, double* pts){
  ...
  if (tv.isValid()){
    ...
    if (muNumber==1) {vertexingTracks(event,iSetup, tv, _Ntracks_SV1,pts);vertexingTracksAdaptive(event,iSetup, tv, _Ntracks_SV1A,_SVA_Mu1T_Redone,pts);}
    if (muNumber==2) {vertexingTracks(event,iSetup, tv, _Ntracks_SV2,pts);vertexingTracksAdaptive(event,iSetup, tv, _Ntracks_SV2A,_SVA_Mu2T_Redone,pts);}
...
void Tau3MuAnalysis_V2::offlineReco(const edm::Event& ev, const edm::EventSetup& iSetup, bool& isOffline,std::vector<TLorentzVector>& TheGenMus, bool& triggered){
  ...
  */
  ...
  _inPV1=isInPV(primaryVertex,BestDiMu[0].innerTrack()->pt());
  _inPV2=isInPV(primaryVertex,BestDiMu[1].innerTrack()->pt());

  _inOPV1=isInOtherPV(ev,primaryVertex,BestDiMu[0].innerTrack()->pt());
  _inOPV2=isInOtherPV(ev,primaryVertex,BestDiMu[1].innerTrack()->pt());

  setMuIdToMu(BestDiMu[0], _isTk1,_isSA1,_isGlb1 ,_isTMO1, _isPF1, _isTight1, _npix1,_nTk1,_nMu1,_q1, _chiTk1,_chiMu1,primaryVertex);
  setMuIdToMu(BestDiMu[1], _isTk2,_isSA2,_isGlb2 ,_isTMO2, _isPF2, _isTight2, _npix2,_nTk2,_nMu2,_q2, _chiTk2,_chiMu2,primaryVertex);
  ...
  if (pitrack.Px() !=0 && pitrack.Py()!=0 ){  //just a way to say that pitrack has been found
    ...
    if (pitrack.Pt() < Mu1Mom.Pt() && pitrack.Pt() < Mu2Mom.Pt()) _TkPt=true; 

    double ptArray[3]={Mu1Mom.Pt(),Mu2Mom.Pt(),pitrack.Pt()};


    vertexingTracks(ev,iSetup, tv, _Ntracks_SV, ptArray);
    vertexingTracksAdaptive(ev,iSetup, tv, _Ntracks_SVA,_SVA_Redone, ptArray);
    _tkHighestW=highestWeight(ev,iSetup,tv,pitrack.Pt());

    vertexingTracks(ev,iSetup, tv3, _Ntracks_SVT, ptArray);
    vertexingTracksAdaptive(ev,iSetup, tv3, _Ntracks_SVTA,_SVTA_Redone, ptArray);
    ...
    AdaptiveVertexFitter avf1;
    TransientVertex atv3=avf1.vertex(vtttot);

    if (atv3.isValid()){
      if (debug) cout << "Adaptive SVT build" << endl;
      _SVTA->SetXYZ(atv3.position().x(),atv3.position().y(),atv3.position().z());
      if (atv3.hasTrackWeight()){
	_w1=atv3.trackWeight(mu1tt);
	_w2=atv3.trackWeight(mu2tt);
	_w3=atv3.trackWeight(pitt);
      }
    }
    ...
    vtx(ev,iSetup,vtt13,_SV_Mu1T,_SVe_Mu1T,_sv1tValid,_SV1chi,_SV1prob,1, ptArray); //fill the vertex of track with single muons
    vtx(ev,iSetup,vtt23,_SV_Mu2T,_SVe_Mu2T,_sv2tValid,_SV2chi,_SV2prob,2, ptArray);

    Vertex muvtx=Vertex(tv);
    Vertex trivtx=Vertex(tv3);
    ...
    std::pair<bool,Measurement1D> ipv1 = IPTools::absoluteImpactParameter3D(mu1tt,primaryVertex);
    std::pair<bool,Measurement1D> ipv2 = IPTools::absoluteImpactParameter3D(mu2tt,primaryVertex);

    if (ipv.first){
      _3D_IP_PV=ipv.second.value();
      if (debug) cout << "3D IP Tk wrt pv " << ipv.second.value() << endl;
      _3D_IPSig_PV=ipv.second.significance();
    }

    if (ipv1.first){
      _3D_IP1_PV=ipv1.second.value();
      if (debug) cout << "3D IP Mu1 wrt pv " << ipv1.second.value() << endl;
      _3D_IP1Sig_PV=ipv1.second.significance();
    }

    if (ipv2.first){
      _3D_IP2_PV=ipv2.second.value();
      if (debug) cout << "3D IP Mu1 wrt pv " << ipv2.second.value() << endl;
      _3D_IP2Sig_PV=ipv2.second.significance();
    }
    ...
    if (isv.first){
      ...
      if (debug) cout << "3D IP Tk wrt sv= " << isv.second.value() << endl;
      _3D_IPSig_SV=isv.second.significance();
    }

    std::pair<bool,Measurement1D> isvt = IPTools::absoluteImpactParameter3D(pitt,trivtx);
    std::pair<bool,Measurement1D> isvt1 = IPTools::absoluteImpactParameter3D(mu1tt,trivtx);
    std::pair<bool,Measurement1D> isvt2 = IPTools::absoluteImpactParameter3D(mu2tt,trivtx);

    if (isvt.first){
      _3D_IP_SVT=isvt.second.value();
      if (debug) cout << "3D IP Tk wrt svt= " << isvt.second.value() << endl;
      _3D_IPSig_SVT=isvt.second.significance();
    }

    if (isvt1.first){
      _3D_IP1_SVT=isvt1.second.value();
      if (debug) cout << "3D IP Mu1 wrt svt= " << isvt1.second.value() << endl;
      _3D_IP1Sig_SVT=isvt1.second.significance();
    }

    if (isvt2.first){
      _3D_IP2_SVT=isvt2.second.value();
      if (debug) cout << "3D IP Mu2 wrt svt= " << isvt2.second.value() << endl;
      _3D_IP2Sig_SVT=isvt2.second.significance();
    }
    ...
    TrackCompatibility(iSetup,primaryVertex,tv3,vtttot,_NtracksPV_SVT, DiMuTrackMom_Pi);

    
    ptAround(ev,tv3,primaryVertex,vtttot,DiMuTrackMom_Mu, 0.3, _Sum03,_Num03);
    ptAround(ev,tv3,primaryVertex,vtttot,DiMuTrackMom_Mu, 0.5, _Sum05,_Num05);
    ptAround(ev,tv3,primaryVertex,vtttot,DiMuTrackMom_Mu, 0.7, _Sum07,_Num07);

    if (debug) cout << " 3Mu Inv. Mass= " <<  DiMuTrackMom_Pi.M() << endl;

    TLorentzVector TotMomArray[3]={Mu1Mom, Mu2Mom, pitrack};
    double ptErrors[3]={BestDiMu[0].innerTrack()->ptError(),BestDiMu[1].innerTrack()->ptError(),thetrack.ptError()};

    double dRcount=0.3;

    _Ntracks_Mu1=countTracksAround(ev,iSetup,TotMomArray,dRcount,tv3,0,_SumPtMu1);
    _Ntracks_Mu2=countTracksAround(ev,iSetup,TotMomArray,dRcount,tv3,1,_SumPtMu2);
    _Ntracks_Mu3=countTracksAround(ev,iSetup,TotMomArray,dRcount,tv3,2,_SumPtMu3);

    bool isEventMatched=false;

    if (IsMC){
      if (!isBkg) isEventMatched=isMcMatched(ev,TotMomArray,TheGenMus);
      else isEventMatched=isBkgMatched(ev,TotMomArray,ptErrors);
    ...
    double cos23D=Compute_CosPointingAngle3D(primaryVertex,tv,DiMuMom);
    double cos33D=Compute_CosPointingAngle3D(primaryVertex,tv3,DiMuTrackMom_Mu);
...
void Tau3MuAnalysis_V2::Initialize_TreeVars(){
  ...
  if (IsMC){
    ...
    if (isBkg){
      ...
      _isTkR1=false; _isGlbR1=false; _isSAR1=false; _isTMOR1=false; _isTightR1=false; _isPFR1=false;
       ...
       _inPVR1=false;_inPVR2=false;_inPVR3=false;
       _inOPVR1=false;_inOPVR2=false;_inOPVR3=false;
      ...
      _SVA_Mu1TR_Redone->SetXYZ(0.,0.,0.); 
      _SVA_Mu2TR_Redone->SetXYZ(0.,0.,0.);
      _SVTAR_Redone->SetXYZ(0.,0.,0.);
      _SVAR_Redone->SetXYZ(0.,0.,0.);
  ...
  _inPV1=false;_inPV2=false;_inPV3=false;
  _inOPV1=false;_inOPV2=false;_inOPV3=false;
  ...
  _SVA_Mu1T_Redone->SetXYZ(0.,0.,0.); 
  _SVA_Mu2T_Redone->SetXYZ(0.,0.,0.);
  _SVTA_Redone->SetXYZ(0.,0.,0.);
  _SVA_Redone->SetXYZ(0.,0.,0.);
...
bool Tau3MuAnalysis_V2::isBkgMatched(const edm::Event& ev,TLorentzVector* recov, double* ptErr){
  ...
  while(runMatch){
	...
	double pull=fabs((recov[r].Pt()-it->pt())/ptErr[r]);

	if (it->status()!=1) continue;

	if (t.DeltaR(recov[r]) < dRtmp && pull < 3){
...
bool Tau3MuAnalysis_V2::matchGenReco(const edm::Event& ev,std::vector<TLorentzVector>& genv, std::vector<std::pair<TLorentzVector,int> >& matches,std::vector<pair<const Track*,int> >& tmatches){
  ...
  while(runMatch){
	...
	double pull=fabs((genv[g].Pt()-t.Pt())/it->ptError());

	if (t.DeltaR(genv[g])<dRtmp && pull < 3){
  ...
  if (debug){
    cout << "Reco-Gen matched objects indexes:" << endl;
    for(uint i=0; i<matches.size();i++) cout << tmatches[i].second << " Pt " << (tmatches[i].first)->pt()<< endl;
  }
  
  if (debug) cout << "End Reco-Gen Match fill" << endl;
  return isMatched;
}

void Tau3MuAnalysis_V2::setMuIdToMu(Muon & mu,bool& isTkMu,bool& isSAMu,bool& isGlbMu,bool& isTMOMu,bool& isPFMu,bool& isTightMu,int& pix,int& tk,int& muh, int& q,double & chiTk,double& chiMu, Vertex& pv){

  if (debug) cout << "Setting mu id to mu" << endl;
  ...
  if (mu.isStandAloneMuon()) {
    isSAMu=true;
    muh=(mu.outerTrack())->hitPattern().numberOfValidMuonHits();
  }
  if (mu.isGlobalMuon()){
    isGlbMu=true;
  ...
  if (isTight(&mu,pv)) isTightMu=true;

  if (debug) cout << "End muId to mu" << endl;
}

void Tau3MuAnalysis_V2::setMuIdToTrack(const edm::Event& ev, const Track* t,bool& isTkMu,bool& isSAMu,bool& isGlbMu,bool& isTMOMu,bool& isPFMu,bool& isTightMu,int& pix,int& tk,int& muh, int& q,double & chiTk,double& chiMu, bool& shares, double& kink, Vertex& pv){

  if (debug) cout << "Setting the mu id to a track:" << endl;
  ...
  for (MuonCollection::const_iterator recoMu = muons->begin();
    ...
    if (mu.isNonnull() && mu.isAvailable()){
      ...
      if (mu->eta() != t->eta()) continue;

      nmatches++;
      if (debug) cout << "the track is a muon: found "  << nmatches << " matches "<< endl;
      if (debug) cout << "tk pt " << t->pt() << " eta " << t->eta() << " phi " << t->phi() <<endl;
      if (debug) cout << "mu pt " << mu->pt() << " eta " << mu->eta() << " phi " << mu->phi() <<endl;

      
      isTkMu=true;
      if (debug) cout << "setting specific muId" << endl;

      if (recoMu->isStandAloneMuon()) {
	isSAMu=true;
	muh=(recoMu->outerTrack())->hitPattern().numberOfValidMuonHits();
      }
	...
	chiMu=recoMu->globalTrack()->normalizedChi2();
      ...
      if (isTight(&*recoMu, pv)) isTightMu=true;
    }
  }
  if (debug) cout << "End muId to track: found: " << nmatches << " tk with muId " << endl;  
}
...
void Tau3MuAnalysis_V2::fillRecoMatchedInfo(const edm::Event& event, const edm::EventSetup& iSetup, std::vector<pair<const Track*,int> >&  matches){
  ...
  if (debug) cout << "Filling Gen-Reco Matched branches" << endl;
  ...
  if (tr.Pt() < m1.Pt() && tr.Pt() < m2.Pt()) _TkPtR=true;

  double ptArray[3]={m1.Pt(),m2.Pt(),tr.Pt()};
  ...
  if (dimuvtx.isValid()){
    ...
    vertexingTracks(event,iSetup, dimuvtx, _Ntracks_SV_R,ptArray);
    vertexingTracksAdaptive(event,iSetup, dimuvtx, _Ntracks_SV_RA,_SVAR_Redone,ptArray);

    _tkHighestWR=highestWeight(event,iSetup,dimuvtx,tr.Pt());
    ...
    pvR=findPV(event,dimuvtx,dimu,_isPVLeadingR);
    ...
    setMuIdToTrack(event,matches[0].first, _isTkR1,_isSAR1,_isGlbR1 ,_isTMOR1, _isPFR1, _isTightR1, _npixR1,_nTkR1,_nMuR1,_qR1, _chiTkR1,_chiMuR1, shares, kink,pvR);
    setMuIdToTrack(event,matches[1].first, _isTkR2,_isSAR2,_isGlbR2 ,_isTMOR2, _isPFR2, _isTightR2, _npixR2,_nTkR2,_nMuR2,_qR2, _chiTkR2,_chiMuR2, shares, kink,pvR);
    setMuIdToTrack(event,matches[2].first, _isTkR3,_isSAR3,_isGlbR3 ,_isTMOR3, _isPFR3, _isTightR3, _npixR3,_nTkR3,_nMuR3,_qR3, _chiTkR3,_chiMuR3, shares, kink,pvR);

    _inPVR1=isInPV(pvR,matches[0].first->pt());
    _inPVR2=isInPV(pvR,matches[1].first->pt());
    _inPVR3=isInPV(pvR,matches[2].first->pt());

    _inOPVR1=isInOtherPV(event,pvR,matches[0].first->pt());
    _inOPVR2=isInOtherPV(event,pvR,matches[1].first->pt());
    _inOPVR3=isInOtherPV(event,pvR,matches[2].first->pt());
    ...
    _cosp23DR=Compute_CosPointingAngle3D(pvR,dimuvtx,dimu);

    std::pair<bool,Measurement1D> ipv = IPTools::absoluteImpactParameter3D(tt3,pvR);
    std::pair<bool,Measurement1D> ipv1 = IPTools::absoluteImpactParameter3D(tt1,pvR);
    std::pair<bool,Measurement1D> ipv2 = IPTools::absoluteImpactParameter3D(tt2,pvR);
    ...
    if (ipv.first){
      ...
      if (debug) cout << "Gen-Reco Matched 3D IP Tk wrt pv " << ipv.second.value() << endl;
      _3D_IPSig_PVR=ipv.second.significance();
    }

    if (ipv1.first){
      _3D_IP1_PVR=ipv1.second.value();
      if (debug) cout << "Gen-Reco Matched 3D IP Mu1 wrt pv " << ipv1.second.value() << endl;
      _3D_IP1Sig_PVR=ipv1.second.significance();
    }

    if (ipv2.first){
      _3D_IP2_PVR=ipv2.second.value();
      if (debug) cout << "Gen-Reco Matched 3D IP Mu2 wrt pv " << ipv2.second.value() << endl;
      _3D_IP2Sig_PVR=ipv2.second.significance();
    }

    if (isv.first){
      _3D_IP_SVR=isv.second.value();
      if (debug) cout << "Gen-Reco Matched 3D IP wrt sv= " << isv.second.value() << endl;
  ...
  if (trimuvtx.isValid()){
    ...
    Vertex trivtx=Vertex(trimuvtx);

    std::pair<bool,Measurement1D> isvt = IPTools::absoluteImpactParameter3D(tt3,trivtx);
    std::pair<bool,Measurement1D> isvt1 = IPTools::absoluteImpactParameter3D(tt1,trivtx);
    std::pair<bool,Measurement1D> isvt2 = IPTools::absoluteImpactParameter3D(tt2,trivtx);

    if (isvt.first){
      _3D_IP_SVTR=isvt.second.value();
      if (debug) cout << "Gen-Reco 3D IP Tk wrt svt= " << isvt.second.value() << endl;
      _3D_IPSig_SVTR=isvt.second.significance();
    }

    if (isvt1.first){
      _3D_IP1_SVTR=isvt1.second.value();
      if (debug) cout << "Gen-Reco 3D IP Mu1 wrt svt= " << isvt1.second.value() << endl;
      _3D_IP1Sig_SVTR=isvt1.second.significance();
    }

    if (isvt2.first){
      _3D_IP2_SVTR=isvt2.second.value();
      if (debug) cout << "Gen-Reco 3D IP Mu2 wrt svt= " << isvt2.second.value() << endl;
      _3D_IP2Sig_SVTR=isvt2.second.significance();
    }

    ptAround(event,trimuvtx,pvR,ttv,trimu, 0.3, _Sum03R,_Num03R );
    ptAround(event,trimuvtx,pvR,ttv,trimu, 0.5, _Sum05R,_Num05R);
    ptAround(event,trimuvtx,pvR,ttv,trimu, 0.7, _Sum07R,_Num07R);

    _Ntracks_Mu1_R=countTracksAround(event,iSetup,TotMomArray,dRcount,trimuvtx,0,_SumPtMu1R);
    _Ntracks_Mu2_R=countTracksAround(event,iSetup,TotMomArray,dRcount,trimuvtx,1,_SumPtMu2R);
    _Ntracks_Mu3_R=countTracksAround(event,iSetup,TotMomArray,dRcount,trimuvtx,2,_SumPtMu3R);

    TrackCompatibility(iSetup,pvR,trimuvtx,ttv,_NtracksPV_SVT_R,trimu);
    ...
    AdaptiveVertexFitter avf1;
    TransientVertex atv3=avf1.vertex(ttv);

    if (atv3.isValid()){
      _SVTAR->SetXYZ(atv3.position().x(),atv3.position().y(),atv3.position().z());
      if (atv3.hasTrackWeight()){
	_w1R=atv3.trackWeight(tt1);
	_w2R=atv3.trackWeight(tt2);
	_w3R=atv3.trackWeight(tt3);
      }
      //cout << " _w1 " << _w1 << " " << _w2 << " " << _w3 << endl;
    }
    ...
    vertexingTracks(event,iSetup, trimuvtx, _Ntracks_SVT_R,ptArray);
    vertexingTracksAdaptive(event,iSetup, trimuvtx, _Ntracks_SVT_RA,_SVTAR_Redone,ptArray);
    ...
    if (!_svValidR)  pvR=findPV(event,trimuvtx,trimu,_isPVLeadingR);
    ...
    _cosp33DR=Compute_CosPointingAngle3D(pvR,trimuvtx,trimu);
  ...
  if (vtx1T.isValid()){
    ...
    vertexingTracks(event,iSetup, vtx1T, _Ntracks_SV1_R,ptArray);
    vertexingTracksAdaptive(event,iSetup, vtx1T, _Ntracks_SV1_RA,_SVA_Mu1TR_Redone,ptArray);
  }
  ...
  if (vtx2T.isValid()){
    ...
    vertexingTracks(event,iSetup, vtx2T, _Ntracks_SV2_R,ptArray);
    vertexingTracksAdaptive(event,iSetup, vtx2T, _Ntracks_SV2_RA,_SVA_Mu2TR_Redone,ptArray);
  }
...
void Tau3MuAnalysis_V2::beginJob() {
  ...
  if (IsMC){
    ...
    if (isBkg){
      ...
      _SVA_Mu1TR_Redone= new TVector3(0.,0.,0.); 
      _SVA_Mu2TR_Redone= new TVector3(0.,0.,0.);
      _SVTAR_Redone= new TVector3(0.,0.,0.);
      _SVAR_Redone= new TVector3(0.,0.,0.);
      ...
      ExTree->Branch("PV_Gen","TVector3",&_GenPV);

      ExTree->Branch("CosPoint2_Gen",&_cosp2G   ,"_cosp2G/D");
      ExTree->Branch("CosPoint3_Gen",&_cosp3G   ,"_cosp3G/D");

      ExTree->Branch("LxyT_Gen",&_lxyG   ,"_lxyG/D");

      ExTree->Branch("CosPoint2_3D_Gen",&_cosp23DG   ,"_cosp23DG/D");
      ExTree->Branch("CosPoint3_3D_Gen",&_cosp33DG   ,"_cosp33DG/D");
      ...
      ExTree->Branch("IsTkLowestPt_Reco",&_TkPtR   ,"_TkPtR/O");

      ExTree->Branch("IsTkHighW_SV_Reco",&_tkHighestWR   ,"_tkHighestWR/O");

      ExTree->Branch("IsInPV_Reco_1",&_inPVR1   ,"_inPVR1/O");
      ExTree->Branch("IsInPV_Reco_2",&_inPVR2   ,"_inPVR2/O");
      ExTree->Branch("IsInPV_Reco_3",&_inPVR3   ,"_inPVR3/O");

      ExTree->Branch("IsInOtherPV_Reco_1",&_inOPVR1   ,"_inOPVR1/O");
      ExTree->Branch("IsInOtherPV_Reco_2",&_inOPVR2   ,"_inOPVR2/O");
      ExTree->Branch("IsInOtherPV_Reco_3",&_inOPVR3   ,"_inOPVR3/O");
      ...
      ExTree->Branch("SumPT_PV_Reco",&_SumPtPVR,"_SumPtPVR/D");
      ...
      ExTree->Branch("NTracksInPv_Reco",& _NtkPVR   ,"_NtkPVR/I");
      ...
      ExTree->Branch("SumPt07_Reco",&_Sum07R,"_Sum07R/D");

      ExTree->Branch("NumPt03_Reco",&_Num03R,"_Num03R/I");
      ExTree->Branch("NumPt05_Reco",&_Num05R,"_Num05R/I");
      ExTree->Branch("NumPt07_Reco",&_Num07R,"_Num07R/I");

      ExTree->Branch("IP3D_SV_Reco",&_3D_IP_SVR   ,"_3D_IP_SVR/D");
      ExTree->Branch("IP3DSig_SV_Reco",&_3D_IPSig_SVR   ,"_3D_IPSig_SVR/D");

      ExTree->Branch("IP3D_SVT_Reco",&_3D_IP_SVTR   ,"_3D_IP_SVTR/D");
      ExTree->Branch("IP3DSig_SVT_Reco",&_3D_IPSig_SVTR   ,"_3D_IPSig_SVTR/D");

      ExTree->Branch("IP3D_PV_Reco",&_3D_IP_PVR   ,"_3D_IP_PVR/D");
      ExTree->Branch("IP3DSig_PV_Reco",&_3D_IPSig_PVR   ,"_3D_IPSig_PVR/D");

      ExTree->Branch("IP3D_SVT_Mu1_Reco",&_3D_IP1_SVTR   ,"_3D_IP1_SVTR/D");
      ExTree->Branch("IP3DSig_SVT_Mu1_Reco",&_3D_IP1Sig_SVTR   ,"_3D_IP1Sig_SVTR/D");

      ExTree->Branch("IP3D_PV_Mu1_Reco",&_3D_IP1_PVR   ,"_3D_IP1_PVR/D");
      ExTree->Branch("IP3DSig_PV_Mu1_Reco",&_3D_IP1Sig_PVR   ,"_3D_IP1Sig_PVR/D");

      ExTree->Branch("IP3D_SVT_Mu2_Reco",&_3D_IP2_SVTR   ,"_3D_IP2_SVTR/D");
      ExTree->Branch("IP3DSig_SVT_Mu2_Reco",&_3D_IP2Sig_SVTR   ,"_3D_IP2Sig_SVTR/D");

      ExTree->Branch("IP3D_PV_Mu2_Reco",&_3D_IP2_PVR   ,"_3D_IP2_PVR/D");
      ExTree->Branch("IP3DSig_PV_Mu2_Reco",&_3D_IP2Sig_PVR   ,"_3D_IP2Sig_PVR/D");
      ...
      ExTree->Branch("SVTAdaptive_Reco","TVector3",&_SVTAR);

      ExTree->Branch("SV_Mu1TrackRedone_Reco","TVector3",&_SVA_Mu1TR_Redone);
      ExTree->Branch("SV_Mu2TrackRedone_Reco","TVector3",&_SVA_Mu2TR_Redone);
      ExTree->Branch("SVT_Redone_Reco","TVector3",&_SVTAR_Redone);
      ExTree->Branch("SV_Redone_Reco","TVector3",&_SVAR_Redone);

      ExTree->Branch("W1_Reco",&_w1R,"_w1R/D");
      ExTree->Branch("W2_Reco",&_w2R,"_w2R/D");
      ExTree->Branch("W3_Reco",&_w3R,"_w3R/D");
      ...
      ExTree->Branch("CosPoint2_3D_Reco",&_cosp23DR   ,"_cosp23DR/D");
      ExTree->Branch("CosPoint3_3D_Reco",&_cosp33DR   ,"_cosp33DR/D");
      ...
      ExTree->Branch("Ntracks_AroundMu3_Reco",&_Ntracks_Mu3_R,"_Ntracks_Mu3_R/I");

      ExTree->Branch("SumPt_AroundMu1_Reco",&_SumPtMu1R,"_SumPtMu1R/D");
      ExTree->Branch("SumPt_AroundMu2_Reco",&_SumPtMu2R,"_SumPtMu2R/D");
      ExTree->Branch("SumPt_AroundMu3_Reco",&_SumPtMu3R,"_SumPtMu3R/D");
      ...
      ExTree->Branch("Ntracks_SV_Reco",&_Ntracks_SV_R,"_Ntracks_SV_R/I");

      ExTree->Branch("Ntracks_SVTAdaptive_Reco",&_Ntracks_SVT_RA,"_Ntracks_SVT_RA/I");
      ExTree->Branch("Ntracks_SV1Adaptive_Reco",&_Ntracks_SV1_RA,"_Ntracks_SV1_RA/I");
      ExTree->Branch("Ntracks_SV2Adaptive_Reco",&_Ntracks_SV2_RA,"_Ntracks_SV2_RA/I");
      ExTree->Branch("Ntracks_SVAdaptive_Reco",&_Ntracks_SV_RA,"_Ntracks_SV_RA/I");
  ...
  _SVA_Mu1T_Redone= new TVector3(0.,0.,0.); 
  _SVA_Mu2T_Redone= new TVector3(0.,0.,0.);
  _SVTA_Redone= new TVector3(0.,0.,0.);
  _SVA_Redone= new TVector3(0.,0.,0.);
  ...
  ExTree->Branch("IsTkLowestPt_Offline",&_TkPt   ,"_TkPt/O");

  ExTree->Branch("IsTkHighW_SV_Offline",&_tkHighestW   ,"_tkHighestW/O");

  ExTree->Branch("IsInPV_Offline_1",&_inPV1   ,"_inPV1/O");
  ExTree->Branch("IsInPV_Offline_2",&_inPV2   ,"_inPV2/O");
  ExTree->Branch("IsInPV_Offline_3",&_inPV3   ,"_inPV3/O");

  ExTree->Branch("IsInOtherPV_Offline_1",&_inOPV1   ,"_inOPV1/O");
  ExTree->Branch("IsInOtherPV_Offline_2",&_inOPV2   ,"_inOPV2/O");
  ExTree->Branch("IsInOtherPV_Offline_3",&_inOPV3   ,"_inOPV3/O");
 ...
 //first offline mu   
  ...
  ExTree->Branch("SumPT_PV_Offline",&_SumPtPV,"_SumPtPV/D");

  ExTree->Branch("NTracksInPv_Offline",& _NtkPV   ,"_NtkPV/I");
 ...
 //third offline mu-track   
  ...
  ExTree->Branch("IP3D_SVT_Offline",&_3D_IP_SVT   ,"_3D_IP_SVT/D");
  ExTree->Branch("IP3DSig_SVT_Offline",&_3D_IPSig_SVT   ,"_3D_IPSig_SVT/D");
  
  ExTree->Branch("IP3D_PV_Offline",&_3D_IP_PV   ,"_3D_IP_PV/D");
  ExTree->Branch("IP3DSig_PV_Offline",&_3D_IPSig_PV   ,"_3D_IPSig_PV/D");

  ExTree->Branch("IP3D_SVT_Mu1_Offline",&_3D_IP1_SVT   ,"_3D_IP1_SVT/D");
  ExTree->Branch("IP3DSig_SVT_Mu1_Offline",&_3D_IP1Sig_SVT   ,"_3D_IP1Sig_SVT/D");

  ExTree->Branch("IP3D_PV_Mu1_Offline",&_3D_IP1_PV   ,"_3D_IP1_PV/D");
  ExTree->Branch("IP3DSig_PV_Mu1_Offline",&_3D_IP1Sig_PV   ,"_3D_IP1Sig_PV/D");

  ExTree->Branch("IP3D_SVT_Mu2_Offline",&_3D_IP2_SVT   ,"_3D_IP2_SVT/D");
  ExTree->Branch("IP3DSig_SVT_Mu2_Offline",&_3D_IP2Sig_SVT   ,"_3D_IP2Sig_SVT/D");

  ExTree->Branch("IP3D_PV_Mu2_Offline",&_3D_IP2_PV   ,"_3D_IP2_PV/D");
  ExTree->Branch("IP3DSig_PV_Mu2_Offline",&_3D_IP2Sig_PV   ,"_3D_IP2Sig_PV/D");
  ...
  ExTree->Branch("Ntracks_AroundMu3_Offline",&_Ntracks_Mu3,"_Ntracks_Mu3/I");

  ExTree->Branch("SumPt_AroundMu1_Offline",&_SumPtMu1,"_SumPtMu1/D");
  ExTree->Branch("SumPt_AroundMu2_Offline",&_SumPtMu2,"_SumPtMu2/D");
  ExTree->Branch("SumPt_AroundMu3_Offline",&_SumPtMu3,"_SumPtMu3/D");
  ...
  ExTree->Branch("Ntracks_SV_Offline",&_Ntracks_SV,"_Ntracks_SV/I");

  ExTree->Branch("Ntracks_SVTAdaptive_Offline",&_Ntracks_SVTA,"_Ntracks_SVTA/I");
  ExTree->Branch("Ntracks_SV1Adaptive_Offline",&_Ntracks_SV1A,"_Ntracks_SV1A/I");
  ExTree->Branch("Ntracks_SV2Adaptive_Offline",&_Ntracks_SV2A,"_Ntracks_SV2A/I");
  ExTree->Branch("Ntracks_SVAdaptive_Offline",&_Ntracks_SVA,"_Ntracks_SVA/I");
 ...
 //Offline vertices
  ...
  ExTree->Branch("SV_Mu1TrackRedone_Offline","TVector3",&_SVA_Mu1T_Redone);
  ExTree->Branch("SV_Mu2TrackRedone_Offline","TVector3",&_SVA_Mu2T_Redone);
  ExTree->Branch("SVT_Redone_Offline","TVector3",&_SVTA_Redone);
  ExTree->Branch("SV_Redone_Offline","TVector3",&_SVA_Redone);
  ...
  ExTree->Branch("SVTAdaptive_Offline","TVector3",&_SVTA);

  ExTree->Branch("W1_Offline",&_w1,"_w1/D");
  ExTree->Branch("W2_Offline",&_w2,"_w2/D");
  ExTree->Branch("W3_Offline",&_w3,"_w3/D");
  ...
  ExTree->Branch("SumPt07_Offline",&_Sum07,"_Sum07/D");

  ExTree->Branch("NumPt03_Offline",&_Num03,"_Num03/I");
  ExTree->Branch("NumPt05_Offline",&_Num05,"_Num05/I");
  ExTree->Branch("NumPt07_Offline",&_Num07,"_Num07/I");

  ExTree->Branch("CosPoint2_Offline",&_cosp2   ,"_cosp2/D");
  ExTree->Branch("CosPoint3_Offline",&_cosp3   ,"_cosp3/D");

  ExTree->Branch("CosPoint2_3D_Offline",&_cosp23D   ,"_cosp23D/D");
  ExTree->Branch("CosPoint3_3D_Offline",&_cosp33D   ,"_cosp33D/D");
  //
...
Tau3MuAnalysis_V2::endJob() {
  ...
  if (IsMC) std::cout << "Wrong Production " << WrongMC << std::endl;
  if (IsMC) std::cout << "Events selected offline and MC matched " << GenMatches  << std::endl;
  cout << "Ntight mu " << Ntight << endl;
}
